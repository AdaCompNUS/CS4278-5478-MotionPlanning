...
class Planner:
    def __init__(self, world_width, world_height, world_resolution, inflation_ratio=3):
        ...
        Arguments:
            ...
            world_resolution {float} -- resolution of map. i.e. How wide in meters each pixel represents
    ...
    """
    -------------------------
    TODO You need to implement the following three functions
    -------------------------
    """
    ...
    def collision_checker(self, x, y):
        ...
        Hint: 1. you should consider the augmented map and the world size
              2. Note that the origin of the map is at its topleft corner and the origin of the world is at bottom left corner.
                 You might need to do the proper conversions.
    ...
    """
    -------------------------
    You are encouraged to use the following helper APIs in your code.
    -------------------------
    """

    def check_goal(self, pose):
        """Simple goal checking criteria, which only requires the current position is less than 0.25 from the goal position.
    ...
    def motion_predict(self, x, y, theta, v, w, dt=0.5, frequency=10):
        ...
        Arguments:
            x {float} -- current x of robot, in world frame
            y {float} -- current y of robot, in world frame
            theta {float} -- current theta of robot
            v {float} -- linear velocity, in robot frame
            w {float} -- angular velocity, in robot frame
        ...
        Returns:
            tuple -- next x, y, theta, in world frame.
                     return None if has collision
        """
    ...
    def discrete_motion_predict(self, x, y, theta, v, w):
        """discrete version of the motion predict. Predict the position of robot after 1 second.
        Note that since the ROS simulation interval is set to be 0.5 sec
        ...
        Arguments:
            x {int} -- current x of robot, in world frame.
            y {int} -- current y of robot, in world frame.
            theta {int} -- current theta of robot, in world frame.
            v {int} -- linear velocity in robot frame. Possible values: (0, 1).
            w {int} -- angular velocity in robot frame. Possible values: (0, 1, -1). 1 for turing left, -1 for turning right.
                       This corresponds to the discrete action definitions in the homework description.

        Returns:
            tuple -- next x, y, theta in world frame. theta = (0, 1, 2, 3). 0: east, 1: north, 2: west, 3: south.
                     return None if has collision
        """
        w_radian = w * np.pi/2
    ...
    """
    ------------------------------
    The following APIs are used for testing and running simulation. Do not modify it.
    ------------------------------
    """
    ...
    def _create_control_msg(self, x, y, z, ax, ay, az):
    ...
    def publish_control(self):
        ...
        for action in self.action_seq:
            msg = self._create_control_msg(action[0], 0, 0, 0, 0, action[1])
    ...
    def publish_stochastic_control(self):
        ...
        while not self.check_goal(current_state):
            current_state = self.get_current_discrete_state()
            ...
            msg = self._create_control_msg(action[0], 0, 0, 0, 0, action[1]*np.pi/2)
            self.controller.publish(msg)
            rospy.sleep(0.6)
            self.controller.publish(msg)
            rospy.sleep(0.6)
...
if __name__ == "__main__":
    ...
    # TODO: You should replace this with other control publishers based on your planners.
    # e.g. For continuous actions, use publish_control() instead.
